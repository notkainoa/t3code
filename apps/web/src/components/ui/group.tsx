...
function GroupText({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "relative inline-flex items-center whitespace-nowrap gap-2 rounded-md border border-input bg-secondary px-3 text-[13px] text-secondary-foreground shadow-xs outline-none transition-shadow [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0",
      className,
    ),
