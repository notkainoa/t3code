...
function resolveBrowserApiCorsAllowedOrigins(config: { readonly devUrl?: URL | undefined }) {
  if (!config.devUrl) {
    return [] as Array<string>;
  }

  const allowedOrigins = new Set<string>([config.devUrl.origin]);
  const devOrigin = new URL(config.devUrl);
  if (isLoopbackHostname(devOrigin.hostname)) {
    for (const hostname of ["localhost", "127.0.0.1", "[::1]"] as const) {
      const loopbackOrigin = new URL(config.devUrl);
      loopbackOrigin.hostname = hostname;
      allowedOrigins.add(loopbackOrigin.origin);
    }
  }
...
export const browserApiCorsLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    return HttpRouter.cors({
      allowedOrigins: resolveBrowserApiCorsAllowedOrigins(config),
